...
class LoadEKittiPointsFromFile(BaseTransform):
    ...
    Args:
        coord_type (str): The type of coordinates of points cloud.
            ...
            - 'DEPTH': Points in depth coordinates, usually for indoor dataset.
            - 'CAMERA': Points in camera coordinates.
        load_dim (int): The dimension of the loaded points. Defaults to 8
            for (x, y, z, r, g, b, class_id, instance_id).
        use_dim (list[int] | int): Which dimensions of the points to use.
            Defaults to [0, 1, 2] for no-color, no-prior use. 
            For eKITTI dataset, set use_dim=8
            or use_dim=[0, 1, 2, 3, 4, 5, 6, 7] to use color dimensions
            as well as a class_id prior and an instance_id prior.
        shift_height (bool): Whether to use shifted height. Defaults to False.
        use_color (bool): Whether to use color features. Defaults to False.
        use_prior_labels (bool): Whether to use prior class & instance labels.
            Defaults to False.
        backend_args (dict, optional): Arguments to instantiate the
            corresponding backend. Defaults to None.
    """
    ...
    def transform(self, results: dict) -> dict:
        ...
        Returns:
            ...
            Added key and value are described below.
                ...
                - points (list[:obj:`BasePoints`]): List of Point clouds data. One Pointcloud
                    for each sensor in the scene.
        """
        # (michbaum) Due to our dict structure and multiple pointclouds per scene, we have to iterate over
        #            all pointclouds and load them individually here.
        results['points'] = []
        for pc_name, pc_dict in results['lidar_points'].items():
            if 'PC' not in pc_name:
                # (michbaum) There are also other keys that don't interest us
                continue
            pts_file_path = pc_dict['lidar_path']
            points = self._load_points(pts_file_path)
            points = points.reshape(-1, self.load_dim)
            points = points[:, self.use_dim]
            attribute_dims = None

            # (michbaum) This is untested since we don't need it
            if self.shift_height:
                floor_height = np.percentile(points[:, 2], 0.99)
                height = points[:, 2] - floor_height
                points = np.concatenate(
            ...
            if self.use_color:
                ...
                if attribute_dims is None:
                    attribute_dims = dict()
                # (michbaum) We always assume x, y, z, r, g, b
                attribute_dims.update(
                    dict(color=[
                        3, # r
                        4, # g
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f'shift_height={self.shift_height}, '
        repr_str += f'use_color={self.use_color}, '
        repr_str += f'use_prior_labels={self.use_prior_labels}, '
        repr_str += f'backend_args={self.backend_args}, '
        repr_str += f'load_dim={self.load_dim}, '
        repr_str += f'use_dim={self.use_dim})'
        return repr_str
...
class LoadEKittiAnnotations3D(LoadAnnotations):
    ...
    # TODO: (michbaum) Same thing, needs adaption if _load_bboxes is adapted    
    def _load_labels(self, results: dict) -> None:
        """Private function to load label annotations.
