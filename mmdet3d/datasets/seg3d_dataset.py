...
@DATASETS.register_module()
class Seg3DEKittiDataset(BaseDataset):
    """Base Class for 3D semantic segmentation dataset.

    This is the base dataset of Extended Kitti dataset.
    ...
    def __init__(self,
        ...
        self.metainfo['seg_label_mapping'] = self.seg_label_mapping
        if not kwargs.get('lazy_init', False):
            # TODO: (michbaum) Might need some changes regarding the lazy_init
            #       or the indexes (might need to use the actual sample_idx)
            self.scene_idxs = np.arange(len(self.data_list)).astype(np.int32)
            # self.data_list = [self.data_list[i] for i in self.scene_idxs]

            # set group flag for the sampler
            if not self.test_mode:
                # (michbaum) Don't really see the reasoning here
                self._set_group_flag()
