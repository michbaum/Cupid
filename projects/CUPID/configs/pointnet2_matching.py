...
# -----------------------PARAMETERS----------------------
matching_range = None # (michbaum) Euclidean range within feature vectors of the instance masks will be matched
...
model = dict(
    ...
    backbone=dict(
        type='PointNet2SASSG',
        in_channels=8,  # [xyz, rgb], should be modified with dataset
        ...
        sa_cfg=dict(
            type='PointSAModule',
            pool_mod='max', # TODO: (michbaum) Try mean as well
            use_xyz=True,
            normalize_xyz=False)),
    neck=dict(
    ...
    decode_head=dict(
        ...
        loss_decode=dict(
            ...
            class_weight=(30, 1), # (michbaum) We have vastly more negative "no_match" samples, so we should balance this
            loss_weight=1.0,
