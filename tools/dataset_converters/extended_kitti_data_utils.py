...
def get_extended_kitti_info_path(idx,
                        prefix,
                        info_type='image_2',
                        file_tail='.png',
                        training=True,
...
def get_image_path(idx,
                   ...
                   file_tail='.png',
                   use_prefix_id=False):
    return get_extended_kitti_info_path(idx, prefix, info_type, file_tail, training,
...
def get_label_path(idx,
                   ...
                   info_type='label_2',
                   use_prefix_id=False):
    return get_extended_kitti_info_path(idx, prefix, info_type, '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_velodyne_path(idx,
                      ...
                      exist_check=True,
                      use_prefix_id=False):
    return get_extended_kitti_info_path(idx, prefix, 'velodyne', '.bin', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_calib_path(idx,
                   ...
                   exist_check=True,
                   use_prefix_id=False):
    return get_extended_kitti_info_path(idx, prefix, 'calib', '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_pose_path(idx,
                  ...
                  exist_check=True,
                  use_prefix_id=False):
    return get_extended_kitti_info_path(idx, prefix, 'pose', '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_timestamp_path(idx,
                       ...
                       exist_check=True,
                       use_prefix_id=False):
    return get_extended_kitti_info_path(idx, prefix, 'timestamp', '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_extended_kitti_image_info(path,
                         training=True,
                         label_info=True,
                         velodyne=True,
                         calib=False,
    ...
    KITTI annotation format version 2:
    {
        ...
        point_cloud: {
            num_features: 4 #TODO: (michbaum) Change accordingly
            velodyne_path: ...
        }
        ...
        annos: {
            ...
            rotation_y: [num_gt] angle array # TODO: (michbaum) Add rotation_x and rotation_z
    ...
    def map_func(idx):
        info = {}
        pc_info = {'num_features': 4} # TODO: (michbaum) Change accordingly
        calib_info = {}

        image_info = {'image_idx': idx}
        annotations = None
        if velodyne:
            pc_info['velodyne_path'] = get_velodyne_path( # TODO: (michbaum) Probably needs changing
                idx, path, training, relative_path)
        image_info['image_path'] = get_image_path(idx, path, training, # TODO: (michbaum) Probably needs changing
                                                  relative_path)
        if with_imageshape:
        ...
        if label_info:
            label_path = get_label_path(idx, path, training, relative_path) # TODO: (michbaum) Probably needs changing
            if relative_path:
                label_path = str(root_path / label_path)
            annotations = get_label_anno(label_path) # TODO: (michbaum) Probably needs changing
        info['image'] = image_info
        info['point_cloud'] = pc_info
        if calib:
            calib_path = get_calib_path( # TODO: (michbaum) Probably needs changing
                idx, path, training, relative_path=False)
            with open(calib_path, 'r') as f:
                lines = f.readlines() # TODO: (michbaum) Double check calib file structure in KITTI, maybe needs changing here
            # TODO: (michbaum) This definitely needs changing, number of cameras is variable for example (in the worst case, could 
            # be passed through, but better if it is just inferred from the lines length)
            ...
            if extend_matrix:
                P0 = _extend_matrix(P0) # TODO: (michbaum) No clue what this does, investigate
                P1 = _extend_matrix(P1)
                P2 = _extend_matrix(P2)
                P3 = _extend_matrix(P3)
            R0_rect = np.array([ # TODO: (michbaum) No clue what the rectification rotation matrix is, investigate, also careful above about the number of cameras -> I think not needed
            ...
            Tr_velo_to_cam = np.array([ # TODO: (michbaum) Need to fix if the pointcloud is world frame = IMU frame or something else, but can be solved export side in fo. Again careful for cameras above
                float(info) for info in lines[5].split(' ')[1:13]
            ]).reshape([3, 4])
            Tr_imu_to_velo = np.array([ # TODO: (michbaum) Same consideration
                float(info) for info in lines[6].split(' ')[1:13]
            ]).reshape([3, 4])
            if extend_matrix:
                Tr_velo_to_cam = _extend_matrix(Tr_velo_to_cam)
                Tr_imu_to_velo = _extend_matrix(Tr_imu_to_velo)
            calib_info['P0'] = P0 # TODO: (michbaum) This whole thing has to change
            calib_info['P1'] = P1
            calib_info['P2'] = P2
            calib_info['P3'] = P3
        ...
        if with_plane: # TODO: (michbaum) Could add plane information, idk if needed/beneficiatl
        ...
        if annotations is not None:
            ...
            add_difficulty_to_annos(info) # TODO: (michbaum) Check if this needs changing
        return info
